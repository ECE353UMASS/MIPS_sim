...
int IFID_flag, IDEX_flag, EXMEM_flag, MEMWB_flag,  WB_flag = 0; //pipeline latch flags
...
void IF(struct instruction inst){
  if(IFID_flag == 0 && sim_cycle <= lines){
    if(c == stgcycle){      //fetching an instruction from memory takes c cycles
      ...
      IFID_flag = 1; //set the IFID latch flag high
      WB_flag = 0; //set WB flag low
...
void ID(){
  if(IDEX_flag == 0 && sim_cycle - 1 <= lines){
    if(stgcycle == 1){
...
void EX(){
  if(EXMEM_flag == 0 && sim_cycle - 2 <= lines){
...
void MEM(){
  if(MEMWB_flag == 0 && sim_cycle - 3 <= lines){
    if (c == stgcycle){
...
void WB(){
  if(WB_flag == 0 && sim_cycle - 4 <= lines){
    if(stgcycle == 1){
